...
public class NeuralAI : AI
{
    ...
    public Matrix<float> FeedForwardPlatonicImages()
    {
        Platonic_manager platonic_Manager = GameObject.Find("Platonic Camera").GetComponent<Platonic_manager>();
        //platonic_Manager.GetSemanticKnowledge();
        Dictionary<GameObject, Matrix<float>> instanceKnowledgeDict = platonic_Manager.instanceKnowledgeDict;
        Matrix<float> combinedHiddenLayerOutput = Matrix<float>.Build.Dense(instanceKnowledgeDict.Count, networkLayerDict["hiddenLayer"].Shape[0]);
        networkLayerDict["inputDriveLayer"].Output = Matrix<float>.Build.Dense(driveStates.Count, 1, 0);
        networkLayerDict["inputBodyLayer"].Output = Matrix<float>.Build.Dense(bodyStates.Count, 1, 0);
        networkLayerDict["inputActionLayer"].Output = Matrix<float>.Build.Dense(actionStates.Count, 1, 0);
        int index = 0;
        foreach (KeyValuePair<GameObject, Matrix<float>> x in instanceKnowledgeDict)
        {
            networkLayerDict["inputVisionRedLayer"].Output = x.Value.Row(0).ToColumnMatrix();
            networkLayerDict["inputVisionGreenLayer"].Output = x.Value.Row(1).ToColumnMatrix();
            networkLayerDict["inputVisionBlueLayer"].Output = x.Value.Row(2).ToColumnMatrix();
            Feedforward();
            combinedHiddenLayerOutput.SetRow(index, networkLayerDict["hiddenLayer"].Output.Column(0));
            index++;
        }
        return combinedHiddenLayerOutput;
    }
    public Matrix<float> FeedForwardDriveStates()
    {
        Matrix<float> combinedHiddenLayerOutput = Matrix<float>.Build.Dense(driveStates.Count, networkLayerDict["hiddenLayer"].Shape[0]);
        networkLayerDict["inputBodyLayer"].Output = Matrix<float>.Build.Dense(bodyStates.Count, 1, 0);
        networkLayerDict["inputActionLayer"].Output = Matrix<float>.Build.Dense(actionStates.Count, 1, 0);
        networkLayerDict["inputVisionRedLayer"].Output = Matrix<float>.Build.Dense(visualInput.Row(0).Count, 1, 0);
        networkLayerDict["inputVisionGreenLayer"].Output = Matrix<float>.Build.Dense(visualInput.Row(1).Count, 1, 0);
        networkLayerDict["inputVisionGreenLayer"].Output = Matrix<float>.Build.Dense(visualInput.Row(2).Count, 1, 0);
        for(int i = 0; i < driveStates.Count; i++)
        {
            Vector<float> tempDriveStates = Vector<float>.Build.Dense(driveStates.Count, 0);
            tempDriveStates[i] = 1;
            networkLayerDict["inputDriveLayer"].Output = tempDriveStates.ToColumnMatrix();
            Feedforward();
            combinedHiddenLayerOutput.SetRow(i, networkLayerDict["hiddenLayer"].Output.Column(0));
        }
        return combinedHiddenLayerOutput;
    }
}